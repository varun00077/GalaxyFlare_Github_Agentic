...
PLANNER_TOOLS: list[dict[str, Any]] = [
    _fn("get_alert", "Full NIDS alert record.", {"alert_id": S("alert id, e.g. alt-2001")}, ["alert_id"]),
    _fn("get_flow", "Flow metadata: bytes, HTTP request, response status, payload snippet.",
        {"flow_id": S("flow id from the alert")}, ["flow_id"]),
    _fn("get_asset", "Asset record: services+versions, criticality, controls. 404 if not in inventory.",
        {"key": S("IP address or hostname")}, ["key"]),
    _fn("lookup_cves", "CVEs for a product version; says if affected and which log indicators prove exploitation.",
        {"product": S("product name exactly as in the asset record, e.g. log4j-core"), "version": S("version string")}, ["product", "version"]),
    _fn("search_logs", "Regex search one log source (web|app|auth|process|outbound|db) on a host.",
    ...
    _fn("search_playbooks", "Retrieve the response playbook / advisory for a signature or situation; it lists the evidence to collect.",
        {"query": S("free text, e.g. 'log4j jndi exploit' or 'host not in inventory'")}, ["query"]),
    _fn("check_allowlist", "Allow-list / watch-list / blocked status of an IP.", {"ip": S("IPv4 address")}, ["ip"]),
    _fn("record_evidence", "Record a decisive finding the tagger missed. Category: post_exploitation|attack_blocked|vulnerable|not_vulnerable|context.",
    ...
    _fn("conclude_investigation",
        "Call when evidence is sufficient or cannot be obtained: verdict, confidence 0-1, 2-3 sentence summary citing evidence ids, missing evidence, proposed response.",
        {
            "verdict": {"type": "STRING", "enum": ["SUCCEEDED", "IN_PROGRESS", "FAILED", "INCONCLUSIVE"], "description": "attack outcome"},
            "confidence": {"type": "NUMBER", "description": "0.0-1.0"},
            "summary": S("summary citing evidence ids"),
            ...
            "action_target": S("IPv4 of the source to block (never a hostname)"),
            "escalation_note": S("what a human must do next, for the ticket"),
        },
